"""
ue_import_wall_jump.py

Imports the two new rows (Wall Slide, Air Down Shot) added to Content/Sprite_sheet_with_wall_jump.png
as a brand new, independent asset chain -- own texture, own sprites, two new flipbooks. Same
"dedicated independent chain" pattern ue_import_holdfire.py already established: deliberately
does NOT touch T_DeathMetalCat_SpriteSheet or any of its existing sprites/flipbooks, since those
are already imported/tuned and re-importing the full combined sheet risks silently shifting
proportions/positions other systems depend on.

The source PNG has a flat blue background with no alpha channel (unlike the other sheets, which
were already alpha-matted before being handed off) -- this script's own preprocessing step
(outside Unreal, via Pillow) cropped just the two new rows out of the full sheet and chroma-keyed
the blue background into a soft alpha falloff, producing Content/DMC_WallJump_transparent.png.
Frame bounding boxes in sprite_frames_wall_jump.json are tight per-frame crops in THAT cropped
image's own coordinate space (same convention as sprite_frames_v2.json/sprite_frames_holdfire.json),
not the original full sheet's coordinates.

Invoke via:
    python send_to_ue.py "exec(open(r'C:\\Users\\calvi\\Desktop\\Projects\\PythonTest\\AgentScripts\\ue_import_wall_jump.py').read())"
"""

import json
import unreal

SHEET_PATH = r"C:\Users\calvi\Desktop\Projects\PythonTest\Content\DMC_WallJump_transparent.png"
FRAMES_JSON = r"C:\Users\calvi\Desktop\Projects\PythonTest\AgentScripts\sprite_frames_wall_jump.json"
...
def import_texture():
    tex_name = "T_DeathMetalCat_WallJump"
    ...
    unreal.log(f"[import] imported Wall Jump texture: {tex_full_path}")
...
def main():
    ...
    unreal.log("=== WALL JUMP IMPORT COMPLETE ===")
